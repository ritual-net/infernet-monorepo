...
import numpy as np
from infernet_ml.utils.codec.vector import encode, encode_vector, decode, decode_vector
...
async def create_testReadsContract(input_list) -> int:
    ...
    processed_output = int(ezkl.felt_to_big_endian(res["processed_outputs"]["poseidon_hash"][0]), 0)
    ...
    tx_hash = await TestReads.constructor(data).transact()
    ...
    data = dict(
    input_data=[input_list], output_data={"rpc": global_config.rpc_url, "calls": calls_to_account}
    )

    json.dump(data, open(pathlib.Path(__file__).parent / "input_onchain.json", "w"))
...
async def ezkl_deploy() -> tuple[AsyncContract,AsyncContract]:
    ...
    rpc = await get_rpc()
    verifier = rpc.get_contract(verif_addr, json.load(open(pathlib.Path(__file__).parent / "evm_verifier.abi")))
    attester = rpc.get_contract(att_addr, json.load(open(pathlib.Path(__file__).parent / "attester.abi")))
...
async def test_completion() -> None:
    ...
    data = encode(["bool","bool","bool", "bytes", "bytes"], [False, True, True, input_bytes, output_bytes])
    ...
    def _assertions(input: bytes, output: bytes, proof: bytes) -> None:
        ...
        assert in_dtype == input_dtype, f"input dtype does't match: {in_dtype} {input_dtype}"
        assert in_shape == input_shape, f"input shape doesnt match: {in_shape} {input_shape}"
        assert np.allclose(in_data.numpy().flatten(), input_data.numpy().flatten()), "input data doesnt match"
        raw_output, processed_output = decode(["bytes", "bytes"], output)
        processed_output = decode(["int256[]"], processed_output)
        assert processed_output[0][0] == processed_output_expected, "processed output is unexpected"
        out_dtype, out_shape, out_data = decode_vector(raw_output)
        assert out_dtype == output_dtype, f"input dtype does't match: {out_dtype} {out_dtype}"
        assert out_shape == output_shape, f"input shape doesnt match: {out_shape} {out_shape}"
        assert np.allclose(out_data.numpy().flatten(), output_data.numpy().flatten()), "input data doesnt match"
        proofs.append(proof)
    ...
    res = await attester.functions.verifyWithDataAttestation(evm_verifier.address, verif_calldata_bytes).call()
